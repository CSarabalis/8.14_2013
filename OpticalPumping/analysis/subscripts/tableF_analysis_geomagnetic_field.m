...
uncert_R_B = 0.005; % uncertainty in coil radius in metres (estimated)
dBdR = i2b(tableF(:,2)/R_Y,'y'); % dB/dR
dBdR = dBdR(i); % sort them into the correct order

% total error in magnetic field
uncert_B = sqrt(uncert_I_B^2*dBdI + uncert_R_B^2*dBdR.^2);

% convert magnetic field error to frequency uncertainty
sig = uncert_B;
...
%Peak1
[p,C,chi2]=linFitWithConstant(x.^2,x,y1.^2,sig);
...
%Peak2
[p,C,chi2]=linFitWithConstant(x.^2,x,y2.^2,sig);
