...
bearth_y = -0.5*p(2)/p(1);  % calculate b_{earth,y} from C/B in chris's notation
uncert_bearth_y = sqrt((0.5/p(1))^2 * (C(2,2) + (p(2)/p(1))^2 * C(1,1)));
...
title('Y coil -- Peak 1 (linear fit)')
gtext({['f_{res}^2 = ' num2str(p(1)) ' b_z^2 + ' num2str(p(2)) ' b_z + ' num2str(p(3))],...
    ['b_{earth,y} = ' num2str(bearth_y) ' \pm ' num2str(uncert_bearth_y) ' G'],...
    ['\chi^2 = ' num2str(chi2)]})
...
clear yrange y_mag_field xlab ylab tit1 tit2 x y1 y2 sig...
    ...
    uncert_B sig1 sig2 grads diffs uncert_bearth_Y
...
ypoly1 = p(1)*xpoly.^2 + p(2)*xpoly + p(3);
bearth_x = -0.5*p(2)/p(1);  % calculate b_{earth,x} from C/B in chris's notation
uncert_bearth_x = sqrt((0.5/p(1))^2 * (C(2,2) + (p(2)/p(1))^2 * C(1,1)));
...
title('X coil -- Peak 1 (linear fit)')
gtext({['f_{res}^2 = ' num2str(p(1)) ' b_z^2 + ' num2str(p(2)) ' b_z + ' num2str(p(3))],...
    ['b_{earth,x} = ' num2str(bearth_x) ' \pm ' num2str(uncert_bearth_x) ' G'],...
    ['\chi^2 = ' num2str(chi2)]})
...
clear xrange x_mag_field xlab ylab tit x y1 y2 sig function_hyperbola a0 i...
    xpoly1 xpoly2 p S bearth_x a1 aerr1 a2 aerr2 C chi2 uncert_bearth_X...
